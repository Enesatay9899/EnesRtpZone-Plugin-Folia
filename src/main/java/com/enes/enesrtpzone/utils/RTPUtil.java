...
public class RTPUtil {
    ...
    private static void doFoliaTeleport(EnesRTPZone pl, Player p, World w, int max, int min, int tryCount) {
        if (tryCount > 15) {
            p.sendMessage(ColorUtil.color(pl.getMessageManager().getPrefix() + pl.getMessageManager().getMessage("safe-area-not-found")));
            return;
        }
                ...
                try {
                    ...
                    exec.invoke(sched, pl, w, x, z, (Runnable) () -> {
                        try {
                            ...
                            p.sendMessage(ColorUtil.color(pl.getMessageManager().getPrefix() + pl.getMessageManager().getMessage("teleporting")));
                        } catch (Exception ex) {
                            pl.getLogger().warning("Teleport error: " + ex.getMessage());
                        }
                    });
                } catch (Exception ex) {
                    pl.getLogger().warning("Schedule error: " + ex.getMessage());
                }
            });
        } catch (Exception ex) {
